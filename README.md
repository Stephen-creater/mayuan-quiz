...
- 可部署为在线网址，使用 GitHub 文件存储保存跨设备进度。
...
当前版本针对期末选择题“只考单选”的复习策略做了二次裁剪：以 `人类社会及其发展规律 / 生产力和生产关系 / 多选题148` 为进度节点，节点之前的题目全部保留；节点之后删除多选题和判断题。这样已经做过的多选、判断、错题和标记仍然保留，后续新刷题集中到单选。

由于考试确定只考单选，系统又把原始题库中此前被低优先级规则筛掉、且当前系统不存在的 206 道单选题补回来了。这些题没有插回原章节，统一放在独立章节：

```text
单选补充题 / 原题库低优先级单选汇总
```

这样可以补齐全部 441 道原始单选题，同时不打乱既有章节专题的刷题进度。
